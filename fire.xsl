...
<xsl:template match="/pokedex">
    <xsl:apply-templates select="//pokemon[type='fire' or type2='fire']" />
</xsl:template>
-->
<!-- Print the name (classification): types -->
<!--
<xsl:template match="pokemon">
    <xsl:value-of select="name" /> (<xsl:value-of select="@classification" />):
    <xsl:apply-templates select="type" /><xsl:text>
</xsl:text>
</xsl:template>
-->
